...
class BinarySearchTree(object):
    """Binary search trees keep their keys in sorted order, following the BST
    property: keys that are less than the parent are found in the left subtree,
    and keys greater are found in the right subtree. Therefore, lookup and
    other operations can use the principle of binary search.
    Time complexity: search, insert, delete: O(log n) on average, O(n) if it
